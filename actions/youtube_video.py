...
_log = logging.getLogger("jarvis.youtube")
...
def _open_url(url: str) -> None:
    try:
        ...
        _log.warning(f"⚠️ open_url failed: {e}")
...
def _scrape_first_video_url(query: str) -> str | None:
    ...
    try:
        ...
        _log.warning(f"⚠️ scrape_first_video_url failed: {e}")
...
def _ask_for_url(prompt_text: str = "YouTube video URL:") -> str | None:
    try:
        ...
        _log.warning(f"⚠️ URL dialog failed: {e}")
        return None
...
def _get_transcript(video_id: str) -> str | None:
    ...
    try:
        ...
        _log.warning(f"⚠️ Transcript fetch failed: {e}")
        return None
...
def _scrape_video_info(video_id: str) -> dict:
    ...
    try:
        ...
        _log.warning(f"⚠️ Info scrape failed: {e}")
        return {}
...
def _scrape_trending(region: str = "TR", max_results: int = 8) -> list[dict]:
    ...
    try:
        ...
        _log.warning(f"⚠️ Trending scrape failed: {e}")
        return []
...
def _handle_play(parameters: dict, player) -> str:
    ...
    _log.debug(f"🔍 Scraping first non-Shorts video for: {query}")

    video_url = _scrape_first_video_url(query)

    if video_url:
        _log.debug(f"▶️ Opening: {video_url}")
        _open_url(video_url)
        return f"Playing: {query}"

    _log.debug(f"⚠️ Scrape failed, opening filtered search page")
    fallback_url = (
        f"https://www.youtube.com/results"
        f"?search_query={quote_plus(query)}"
        f"&sp={_YT_VIDEO_FILTER}"
...
def youtube_video(
    ...
    _log.debug(f"▶️  Action: {action}  Params: {params}")
    ...
    try:
        ...
        _log.error(f"❌ Error in {action}: {e}")
