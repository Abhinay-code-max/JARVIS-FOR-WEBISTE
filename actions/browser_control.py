...
_log = logging.getLogger("jarvis.browser")
...
def _real_profile_dir(browser: str) -> str:
    ...
    for p in candidates:
        if p.exists():
            _log.debug(f"✅ Real profile found for {browser}: {p}")
            return str(p)

    fallback = home / ".jarvis_profiles" / browser
    fallback.mkdir(parents=True, exist_ok=True)
    _log.debug(f"⚠️  Real profile not found for {browser}, using: {fallback}")
    return str(fallback)
...
def _firefox_profile_dir() -> Optional[str]:
    ...
    if default_path and Path(default_path).exists():
        _log.debug(f"Firefox real profile: {default_path}")
        return default_path
    return None
            ...
            for hive in (winreg.HKEY_LOCAL_MACHINE, winreg.HKEY_CURRENT_USER):
                try:
                    ...
                    if exe and Path(exe).exists():
                        _log.debug(f"Opera found via registry: {exe}")
                        return exe
                except Exception:
                    continue
    except Exception:
...
def _resolve_browser(name: str) -> dict | None:
    ...
    if spec.get("special") == "opera_windows":
        ...
        if not exe:
            _log.debug(f"⚠️  Opera executable not found on Windows.")
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            ...
            try:
                ...
                _log.warning(f"Firefox real profile failed ({e}), using JARVIS profile")
        ...
        try:
            ...
            await asyncio.sleep(0.5) 
            self._page = await self._context.new_page()
            _log.debug(f"✅ Launched [{label}] profile={profile}")
            return
        except Exception as e:
            _log.warning(f"⚠️  Real profile failed for {label}: {e}")

        jarvis_profile = str(Path.home() / ".jarvis_profiles" / self.browser_name)
        Path(jarvis_profile).mkdir(parents=True, exist_ok=True)
        _log.debug(f"Retrying with JARVIS profile: {jarvis_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(jarvis_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            _log.debug(f"✅ Launched [{label}] with JARVIS profile")
    ...
    def _get_or_create(self, browser_name: str) -> _BrowserSession:
        with self._lock:
            if browser_name not in self._sessions:
                ...
                _log.debug(f"New session: {browser_name}")
